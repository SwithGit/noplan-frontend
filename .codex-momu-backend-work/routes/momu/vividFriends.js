...
const getS3Config = () => {
  const config = {
    ...
    keyPrefix: (process.env.AWS_S3_VIVID_FRIENDS_PREFIX ||
      'momu/vivid-friends/completed').replace(/^\/+|\/+$/g, '')
  };
  ...
  return getSignedUrl(
    ...
    new GetObjectCommand({
      ...
      ResponseContentDisposition:
        `attachment; filename="VividFriends-${dateStamp}.png"`,
  ...
  async (req, res) => {
    try {
      ...
      const qrLink = await createQrDownloadLink({
        objectKey,
        downloadName: `VividFriends-${now.toISOString().slice(0, 10).replace(/-/g, '')}.${extension}`,
        contentType
      });
      ...
      console.error('VividFriends upload error:', error);
