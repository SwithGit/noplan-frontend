    ...
    <section className="landing-section landing-how" id="how-it-works">
      <div className="landing-container">
        <div className="landing-section-heading landing-section-heading-center" data-reveal>
          ...
          <h2>{uiText("장소를 찾고, 이해하고, 연결합니다.")}</h2>
        </div>
        <div className="landing-how-layout">
          <div className="landing-step-tabs" data-reveal-stagger role="tablist" aria-label={uiText("NoPlan 서비스 작동 방식")}>
            {steps.map((step, index) => (
              ...
              >
                ...
                <div><strong>{String(index + 1).padStart(2, '0')} · {uiText(step.title)}</strong><small>{uiText(step.description)}</small></div>
              </button>
            ))}
          </div>
          ...
          <div className="landing-service-stage" data-reveal aria-live="polite">
            ...
            <p><strong>{uiText(steps[activeStep].title)}</strong>{uiText(steps[activeStep].description)}</p>
          </div>
        </div>
      </div>
    </section>
      ...
      <section className="landing-problem">
        <div className="landing-container landing-problem-grid">
          <div className="landing-section-heading" data-reveal>
            ...
            <h2>{uiText("저장한 장소는 많은데,")}<br /><em>{uiText("막상 어디 갈지는 어렵습니다.")}</em></h2>
          </div>
          <div className="landing-problem-list" data-reveal-stagger>
            {problems.map(([number, title, description]) => (
              <article key={number}>
                <span>{number}</span>
                <div><strong>{uiText(title)}</strong><p>{uiText(description)}</p></div>
              </article>
            ))}
          </div>
        </div>
        ...
        <div className="landing-container">
          ...
          <div className="landing-compare-grid">
            <article className="landing-compare-card landing-compare-old" data-reveal>
              <span>{uiText("기존 검색")}</span>
              <h3>{uiText("장소를 하나씩 찾고 조합하기")}</h3>
              <ul><li>{uiText("검색어와 평점 중심")}</li><li>{uiText("장소를 하나씩 확인")}</li><li>{uiText("이동 순서를 직접 구성")}</li></ul>
            </article>
            ...
            <article className="landing-compare-card landing-compare-noplan landing-reveal-delay-1" data-reveal>
              <span>NoPlan</span>
              <h3>{uiText("지금의 조건을 하나의 코스로")}</h3>
              <ul><li>{uiText("상황과 취향을 함께 반영")}</li><li>{uiText("서울 전역의 장소와 공개 코스를 탐색")}</li><li>{uiText("여러 장소를 하나의 코스로 연결")}</li></ul>
            </article>
          </div>
        </div>
      </section>
      ...
      <section className="landing-section landing-local" id="local-content">
        <div className="landing-container landing-local-grid">
          <div className="landing-section-heading" data-reveal>
            ...
            <h2>{uiText("추천하기 전에,")}<br /><em>{uiText("먼저 지역을 들여다봅니다.")}</em></h2>
            <p>{uiText("서울 전역에서 확인하고 등록한 장소와 코스를 바탕으로 사용자의 상황에 맞는 선택지를 연결합니다.")}</p>
          </div>
          <div className="landing-local-visual" aria-label={uiText("서울 전역의 장소를 코스로 연결하는 과정")} data-reveal>
            <div className="landing-map-line" aria-hidden="true" />
            <article><span>1</span><strong>{uiText("장소 탐색")}</strong><small>{uiText("맛집 · 놀거리 · 카페")}</small></article>
            <article><span>2</span><strong>{uiText("조건 확인")}</strong><small>{uiText("동행 · 목적 · 분위기")}</small></article>
            <article><span>3</span><strong>{uiText("코스 연결")}</strong><small>{uiText("이동 순서와 장소 정보")}</small></article>
          </div>
        </div>
      </section>
      ...
      <section className="landing-partnership" id="partnership">
        <div className="landing-container landing-partnership-card" data-reveal>
          <div>
            ...
            <h2>{uiText("지역의 좋은 공간이")}<br />{uiText("더 잘 발견되는 방법을 함께 만듭니다.")}</h2>
            <p>{uiText("공간, 상권, 지역 콘텐츠와 데이터 분야의 파트너를 기다립니다.")}</p>
          </div>
          <span className="landing-coming-soon" aria-label={uiText("파트너십 문의 채널 준비 중")}>{uiText("문의 준비 중")}</span>
        </div>
      </section>
    </>
  );
