  ...
  return (
    <div style={{ display: 'flex', justifyContent: 'center', paddingTop: '30px', paddingBottom: '50px' }}>
      <div style={{ padding: '40px', backgroundColor: 'white', borderRadius: '20px', boxShadow: '0 4px 20px rgba(0,0,0,0.1)', width: '400px', display: 'flex', flexDirection: 'column' }}>
        <h2 style={{ color: '#007AFF', textAlign: 'center', marginBottom: '30px' }}>{uiText("노플랜에 오신 걸 환영해요! ✈️")}</h2>

        <input type="text" placeholder={uiText("이름")} value={name} onChange={(e) => setName(e.target.value)} style={inputStyle} />
        
        <SignupDemographics birthdate={birthdate} gender={gender} onBirthdate={setBirthdate} onGender={setGender} />

        <input type="tel" placeholder={uiText("핸드폰 번호 (예: 010-1234-5678)")} value={phone} onChange={(e) => setPhone(e.target.value)} style={inputStyle} />
        <input type="email" placeholder={uiText("이메일 주소")} value={email} onChange={(e) => setEmail(e.target.value)} style={inputStyle} />
        <input type="text" placeholder={uiText("아이디")} value={id} onChange={(e) => setId(e.target.value)} style={inputStyle} />
        <input type="password" minLength={8} maxLength={128} placeholder={uiText("비밀번호 (8자 이상)")} value={pw} onChange={(e) => setPw(e.target.value)} style={inputStyle} />
        <input type="text" placeholder={uiText("노플랜에서 쓸 닉네임")} value={nickname} onChange={(e) => setNickname(e.target.value)} style={inputStyle} />

        {/* 🎒 코아의 특별 추천! 여행 스타일 고르기 */}
        <p style={{ fontWeight: 'bold', fontSize: '14px', marginBottom: '10px', marginTop: '10px' }}>{uiText("당신의 여행 스타일은?! 🎒")}</p>
        ...
        <label style={{ display: 'flex', alignItems: 'center', marginBottom: '20px', fontSize: '14px', cursor: 'pointer' }}>
          <input type="checkbox" checked={agreeTerms} onChange={(e) => setAgreeTerms(e.target.checked)} style={{ marginRight: '10px', cursor: 'pointer' }} />{uiText("[필수] 개인정보 수집 및 이용에 동의할게요!")}</label>

        <button onClick={handleSignup} style={{ padding: '15px', backgroundColor: '#007AFF', color: 'white', border: 'none', borderRadius: '10px', fontWeight: 'bold', fontSize: '16px', cursor: 'pointer', marginBottom: '15px' }}>{uiText("가입 완료하기 🚀")}</button>

        {/* 로그인 화면으로 돌아가는 센스 있는 버튼! */}
        <p style={{ textAlign: 'center', fontSize: '14px', color: '#888', cursor: 'pointer' }} onClick={onGoToLogin}>{uiText("이미 계정이 있나요? 로그인하러 가기!")}</p>
      </div>
    </div>
  );
}
