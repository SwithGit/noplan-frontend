...
import { createTrip, newId, readDrafts, writeDraft, usedMinutes, minutes, type TripRecord, type TripPlace } from '../trips/tripModel';
...
export function AddEventToTrip({event,user,onClose}:{event:TravelEvent;user:UserSession|null;onClose:()=>void}){
  ...
  const add=()=>{
    ...
    const place:TripPlace={id:newId(),name:event.title,address:event.address||event.venue,type:eventKinds[event.kind],lat:event.lat,lng:event.lng,durationMinutes:duration,fixed:true,source:'manual',sourceUrl:event.sourceUrl,event:{id:event.id,startDate:event.startDate,endDate:event.endDate,hours:event.hours}};
    ...
    if(tripId==='new'){
      ...
      // Choose a visit time in the editor: do not invent a timed ticket from free-form provider text.
      selectedDay=next.document.days[0].id;selectedBlock=next.document.days[0].blocks[1].id;
      next.document.days[0].blocks[1].places=[place];
    }else{
