...
function EventDetailContent({id,user}:{id:string;user:UserSession|null}){
  ...
  return <div className="events-page event-detail"><nav className="event-breadcrumb"><Link to={ROUTES.events}>{uiText("축제·전시")}</Link><span>/</span><span>{eventKinds[event.kind]}</span></nav><div className="event-detail-layout"><div className="event-detail-poster"><EventImage event={event}/><p>{uiText(event.sourceLabel)}{event.imageLicense?` · ${event.imageLicense}`:''}</p></div><section className="event-detail-copy"><div className="event-detail-tags"><span>{eventKinds[event.kind]}</span><span>{event.region}</span><span>{status}</span></div><h1>{uiText(event.title)}</h1><p className="event-detail-date"><TripIcon name="calendar"/>{eventDate(event.startDate)} — {eventDate(event.endDate)}</p>
    {event.detailUnavailable&&<p className="event-notice">{uiText("상세 정보를 갱신하지 못했어요. 현재 확인된 기본 정보를 보여드려요.")}</p>}
    <dl className="event-facts"><div><dt>{uiText("장소")}</dt><dd>{uiText(event.venue||event.address||'공식 안내 확인 필요')}</dd></div><div><dt>{uiText("관람 시간")}</dt><dd>{uiText(event.hours||'공식 안내 확인 필요')}</dd></div><div><dt>{uiText("이용 요금")}</dt><dd>{uiText(event.price||(event.isFree===true?'무료':'공식 안내 확인 필요'))}</dd></div><div><dt>{uiText("관람 대상")}</dt><dd>{uiText(event.age||'공식 안내 확인 필요')}</dd></div>{event.phone&&<div><dt>{uiText("문의")}</dt><dd>{event.phone}</dd></div>}</dl>
    ...
    <div className="event-detail-bottom"><section><span className="event-eyebrow">ABOUT THIS EXPERIENCE</span><h2>{uiText("어떤 경험이 기다릴까요?")}</h2><p className="event-description">{uiText(event.description||'자세한 프로그램은 공식 안내에서 확인해 주세요.')}</p></section><aside><h2>{uiText("찾아가는 길")}</h2><p>{event.venue||event.title}</p><p>{event.address}</p><a className="trip-button" href={`https://map.kakao.com/link/search/${encodeURIComponent(`${event.venue||event.title} ${event.address}`)}`} target="_blank" rel="noreferrer"><TripIcon name="pin"/>{uiText("지도에서 위치 확인")}</a><p className="event-help">{uiText("전국 행사 탐색이 가능하며, AI 주변 코스 추천은 현재 서울·도보 기준이에요.")}</p></aside></div>
