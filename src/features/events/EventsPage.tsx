...
export function EventsPage(){
  ...
  return <div className={`events-page ${!desktop?'mobile-events':''} ${filtersOpen?'filters-open':''}`}>
    {!desktop&&<header className="mobile-events-heading"><span>CULTURE & EVENTS</span><h1>{uiText("문화·행사")}</h1><p>{uiText("전시 한 편, 축제 하루.")}<br/>{uiText("오늘의 여행에 새로운 경험을 더해요.")}</p></header>}
    ...
    <section className="event-search-panel" aria-label={uiText("축제·전시 검색")}>
      ...
      <form className={`event-search-row ${nearby?'nearby-mode':''}`} onSubmit={e=>{e.preventDefault();update({q:search});}}>
        <label className="event-region-field">{uiText("지역")}<select aria-label={uiText("행사 지역")} value={params.get('region')||''} onChange={e=>update({region:e.target.value})}><option value="">{uiText("전국")}</option>{regions.map(region=><option key={region} value={region}>{region}</option>)}</select></label>
        <label className="event-date-field">{uiText(!desktop&&!filtersOpen?'방문일':'방문 시작일')}<input aria-label={uiText("행사 시작일")} type="date" min={koreaToday()} value={from} onChange={e=>update({from:e.target.value,...(!desktop&&!filtersOpen||to&&e.target.value>to?{to:e.target.value}:{})})}/></label>
        <label className="event-date-field event-date-end">{uiText("방문 종료일")}<input aria-label={uiText("행사 종료일")} type="date" min={from||koreaToday()} value={to} onChange={e=>update({to:e.target.value,...(from&&e.target.value&&e.target.value<from?{from:e.target.value}:{})})}/></label>
    ...
    <div className="event-result-heading"><p>{loading?'행사를 찾고 있어요…':<>{uiText(nearby?'코스 주변에서 만날':!desktop&&from===koreaToday()&&to===from?'오늘 만날 수 있는':'지금부터 만날 수 있는')} <strong>{result?.total||0}{uiText("개의 경험")}</strong></>}</p><span>{uiText("기간 중 표시와 실제 입장 가능 여부는 달라요.")}</span></div>
    ...
    {loading?<div className="event-grid" role="status" aria-label={uiText("행사 불러오는 중")}>{Array.from({length:6},(_,i)=><div className="event-skeleton" key={i}><div/><span/><span/></div>)}</div>:error?<div className="event-empty" role="alert"><TripIcon name="calendar"/><h2>{uiText("잠시, 행사를 불러오지 못했어요")}</h2><p>{uiText(error)}</p><button className="trip-button primary" onClick={()=>setRevision(v=>v+1)} type="button">{uiText("다시 불러오기")}</button></div>:result?.events.length?<><div className="event-grid">{result.events.map(event=><Link className="event-card" to={eventRoute(event.id)} key={event.id}><div className="event-card-poster"><EventImage event={event}/><span className={`event-status ${eventStatus(event)==='기간 중'?'ongoing':''}`}>{eventStatus(event)}</span></div><div className="event-card-copy"><div className="event-card-meta"><span>{eventKinds[event.kind]}</span><span>{uiText(event.district||event.region||'지역 확인')}</span>{event.isFree===true&&<span>{uiText("무료")}</span>}</div><h3>{uiText(event.title)}</h3>{event.nearby&&<p className="event-nearby-distance">{event.nearby.placeIndex}{uiText("번째 장소에서 직선 ")}{eventDistance(event.nearby.distanceMeters)}</p>}<p className="event-dates"><TripIcon name="calendar"/>{eventDate(event.startDate)} — {eventDate(event.endDate)}</p><p className="event-venue"><TripIcon name="pin"/>{uiText(event.venue||event.address||'장소 안내 확인')}</p><div className="event-card-footer"><small>{uiText(event.sourceLabel)}</small><span>{uiText("자세히 보기 ")}<TripIcon name="arrow"/></span></div></div></Link>)}</div><nav className="event-pagination" aria-label={uiText("행사 페이지")}><button className="trip-button" type="button" disabled={result.page===1} onClick={()=>update({page:String(result.page-1)})}>{uiText("이전")}</button><span>{result.page} / {Math.ceil(result.total/result.pageSize)}</span><button className="trip-button" type="button" disabled={result.page*result.pageSize>=result.total} onClick={()=>update({page:String(result.page+1)})}>{uiText("다음")}</button></nav></>:<div className="event-empty"><TripIcon name="calendar"/><h2>{uiText(nearby?'코스 주변에 맞는 행사가 아직 없어요':'조건에 맞는 행사가 아직 없어요')}</h2><p>{uiText("다른 날짜나 지역으로 찾아보세요.")}</p><button className="trip-button" type="button" onClick={reset}>{uiText(desktop?'전체 행사 보기':'기본 조건으로 보기')}</button></div>}
    {result&&<footer className="event-source-note">{uiText("한국관광공사 TourAPI · 서울문화포털 제공. 제공기관에 등록된 행사 기준이며 전체 민간 전시·팝업을 포함하지는 않아요.")}<br/>{uiText(result.sources.filter(s=>s.fetchedAt).map(s=>`${s.provider==='tourapi'?'관광공사':'서울시'} 확인: ${new Date(s.fetchedAt!).toLocaleString('ko-KR')}`).join(' · '))}</footer>}
  </div>;
}
