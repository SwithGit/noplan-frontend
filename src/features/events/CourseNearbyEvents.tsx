...
export function CourseNearbyEvents({places}:{places:CoursePlace[]}){
  ...
  return <section className="course-nearby-events" aria-label={uiText("코스 주변 문화·행사")}>
    ...
    <div className="nearby-event-list">{response.events.map(event=><Link className="nearby-event-card" key={event.id} to={eventRoute(event.id)}><EventImage event={event}/><div><small>{eventKinds[event.kind]} · {event.nearby?.placeIndex}{uiText("번째 장소에서 직선 ")}{eventDistance(event.nearby?.distanceMeters||0)}</small><h3>{uiText(event.title)}</h3><p>{eventDate(event.startDate)} — {eventDate(event.endDate)}</p><span>{event.venue||event.address}</span></div><TripIcon name="arrow"/></Link>)}</div>
