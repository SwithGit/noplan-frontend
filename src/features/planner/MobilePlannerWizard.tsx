...
export function MobilePlannerWizard({review=false,DateSheet,AddressSheet}:Props){
  ...
  const value=condition.accuracy||{},fill=normalizeFillPreferences({...value,allowBudgetWalk:true}),selections=selectionsOf(condition.mood),complete=wizardComplete(condition);
  ...
  const advance=async()=>{
    ...
    setBusy(true);
    const next:PlannerCondition={...condition,accuracy:{...value,...normalizeFillPreferences({...value,allowBudgetWalk:true}),preferredFoodDetails:foodValue.preferredFoodDetails},rawText:[condition.location,condition.time,condition.companion,condition.mood,condition.duration].join(' ')};
    setCondition(next);
  ...
  const toggle=(label:string,key:'fillSchedule',enabled:boolean)=><div className="mw-toggle"><span>{label}{infoButton('fill')}</span><div>{[true,false].map(on=><button type="button" key={String(on)} aria-pressed={enabled===on} onClick={()=>patchAccuracy({[key]:on})}>{on?'✓ 허용':'안 함'}</button>)}</div></div>;
