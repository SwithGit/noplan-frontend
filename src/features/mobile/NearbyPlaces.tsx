...
export function NearbyPlaces() {
  ...
  return <div className="mobile-page nearby-page">
    <header className="nearby-header"><Link to={ROUTES.appHome} aria-label="홈으로">‹</Link><div><small>우리 동네 발견</small><h1>주변 {category.label}{category.key === 'hotplace' ? '' : ' 순위'}</h1></div></header>
    <form className="nearby-location" onSubmit={chooseArea}><label htmlFor="nearby-area">어디를 둘러볼까요?</label><div><input id="nearby-area" value={input} onChange={event => setInput(event.target.value)} placeholder="예: 홍대입구역, 성수동" maxLength={80}/><button type="submit">지역 선택</button></div><button type="button" disabled={locating} onClick={() => void locate()}>{locating ? '현재 위치 확인 중…' : '현재 위치로 찾기'}</button></form>
    {notice && <p className="m-notice" role="status">{notice}</p>}
    <nav className="nearby-categories" aria-label="장소 카테고리">{categories.map(item => <button key={item.key} type="button" aria-pressed={category.key === item.key} onClick={() => setParams({ category: item.key })}>{item.label}</button>)}</nav>
    <div className="nearby-intro"><h2>{area || '내 주변'} <span>반경 2km</span></h2><p>{category.key === 'hotplace' ? '가까운 순으로 모았어요. 산책·명소에는 별점과 후기 수를 표시하지 않아요.' : '노플랜 DB 평점에 후기 수를 반영해 정렬했어요. 점수가 같으면 후기 수와 거리를 비교해요.'}</p></div>
    {loading ? <p className="m-inline-empty" role="status">주변 장소를 확인하고 있어요…</p> : error ? <div className="m-inline-empty" role="alert"><p>{error}</p><button type="button" onClick={() => setRetry(value => value + 1)}>다시 불러오기</button></div> : !area ? <p className="m-inline-empty">현재 위치를 사용하거나 동네를 직접 선택해 주세요.</p> : !places.length ? <p className="m-inline-empty">이 지역 2km 안에는 등록된 {category.label} 장소가 아직 없어요. 다른 동네를 선택해 주세요.</p> : <ol className="nearby-list">{places.map(place => <li key={place.id}><button type="button" className="nearby-place" onClick={() => setDetail(place)}><span className="nearby-rank">{place.rank}</span><PlaceVisual imageUrl={place.imageUrl} type={place.type} detailType={place.detailType} alt=""/><span className="nearby-place-copy"><small>{place.detailType || category.label} · 직선 {place.distanceMeters < 1000 ? `${place.distanceMeters}m` : `${(place.distanceMeters / 1000).toFixed(1)}km`}</small><strong>{place.name}</strong><span>{place.address}</span>{category.key !== 'hotplace' && <span className="nearby-rating">{place.rating != null && place.rating > 0 ? `★ ${place.rating.toFixed(1)}` : '별점 미확인'} · {place.reviewCount != null ? `후기 ${place.reviewCount.toLocaleString()}개` : '후기 수 미확인'}</span>}</span><span aria-hidden="true">›</span></button></li>)}</ol>}
    <p className="nearby-footnote">등록된 장소 기준 · 영업시간과 메뉴는 방문 전 확인해 주세요.</p>
