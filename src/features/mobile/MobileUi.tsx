...
export function MobileHeading({eyebrow,title,description,image}:{eyebrow:string;title:ReactNode;description:string;image?:string}) {return <header className="m-heading"><div><span>{uiText(eyebrow)}</span><h1>{uiText(title)}</h1><p>{uiText(description)}</p></div><img src={image || (getLocale()==='ko'?nopi:nopiIcon)} alt="" /></header>;}
...
export function FavoriteButton({item,compact=false}:{item:FavoriteInput;compact?:boolean}) {
  ...
  return <div className={`m-favorite-control ${compact?'compact':''}`}><button className={`m-heart ${selected?'selected':''}`} aria-label={uiText(`${item.title} ${selected?'찜 해제':'찜하기'}`)} aria-pressed={selected} disabled={Boolean(user)&&(loading||busy)} type="button" onClick={()=>{setMessage('');void toggle(item).catch(cause=>setMessage(cause instanceof Error?cause.message:'찜을 저장하지 못했어요.'));}}><MobileIcon name="heart"/>{!compact&&<span>{uiText(selected?'찜했어요':'찜하기')}</span>}</button>{message&&<div className="m-save-message" role="status"><span>{uiText(message)}</span>{!user?<Link to={ROUTES.login}>{uiText("로그인")}</Link>:error?<button type="button" onClick={reload}>{uiText("다시 불러오기")}</button>:null}<button type="button" aria-label={uiText("찜 안내 닫기")} onClick={()=>setMessage('')}>{uiText("닫기")}</button></div>}</div>;
}
