...
export function MobileExplore({active}:{active:boolean}) {
  ...
  return <div className="mobile-page m-explore"><MobileHeading eyebrow="둘러보기" title={uiText(<>지역별<br/><em>인기 코스</em></>)} description="동네마다 다른 여행을 만나보세요."/>
    ...
    <div className="m-filter-scroll" aria-label={uiText("코스 테마")}>{['전체',...categories.map(item=>item.label)].map(value=><button className={theme===value?'selected':''} aria-pressed={theme===value} key={value} type="button" onClick={()=>setTheme(value)}>{value}</button>)}</div>
