...
export function MobileExplore({active}:{active:boolean}) {
  ...
  return <div className="mobile-page m-explore"><MobileHeading eyebrow={uiText("둘러보기")} title={uiText(<>{uiText("내 주변")}<br/><em>{uiText("인기 코스")}</em></>)} description={uiText("동네마다 다른 여행을 만나보세요.")}/>
    ...
    <div className="m-list-top"><span>{uiText(dong||'모든 동네')} · {nearbyAvailable?visible.length:sharedVisible.length}{uiText("개의 코스")}</span><label><span className="m-sr-only">{uiText("코스 정렬")}</span><select aria-label={uiText("코스 정렬")} value={sort} onChange={event=>{setSort(event.target.value as 'likes'|'latest');}}><option value="likes">{uiText("좋아요 많은 순")}</option><option value="latest">{uiText("최신 공개순")}</option></select></label></div>
    <section className="m-discovery-section"><div className="m-section-title"><h2><MobileIcon name="pin"/>{uiText("내 주변 추천 코스")}</h2><span>{uiText("시작점 3km 이내 · 가까운 순")}</span></div>
    {!nearbyAvailable?<div className="m-discovery-empty"><p>{uiText("동네를 선택하면 주변의 공개 코스를 볼 수 있어요.")}</p><button type="button" onClick={()=>setEditor(true)}>{uiText("동네 선택하기")}</button></div>:loading?<div className="m-skeleton" role="status">{uiText("여행 코스를 불러오고 있어요…")}</div>:error?<EmptyState title={uiText("코스를 불러오지 못했어요")} action={<button type="button" className="m-primary" onClick={()=>{setRevision(value=>value+1);}}>{uiText("다시 불러오기")}</button>}>{uiText(error)}</EmptyState>:visible.length?<div className="m-card-list">{visible.map(course=><MobileCourseCard key={course.id} item={courseFavorite(course)} byline={course.distanceMeters!=null?`시작점까지 직선 ${(course.distanceMeters/1000).toFixed(1)}km`:undefined} onOpen={()=>setDetail(courseFavorite(course))}/>)}</div>:<EmptyState title={uiText(query||theme!=='전체'?'조건에 맞는 코스가 없어요':'아직 공개된 코스가 없어요')} action={<button type="button" onClick={()=>{setQuery('');setTheme('전체');setManual(dong);setEditor(true);}}>{uiText("조건·동네 바꾸기")}</button>}>{uiText("다른 테마나 가까운 동네를 살펴보세요.")}</EmptyState>}
    </section>
