...
export function MobileExplore({active}:{active:boolean}) {
  const {condition,detectCurrentLocation,ensureCurrentLocation}=usePlanner();
  const sharedDong=extractDongFromText(condition.locationLabel,condition.location);
