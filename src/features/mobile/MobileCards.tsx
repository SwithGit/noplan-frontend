...
export function MobileCourseCard({item,onOpen,unavailable=false}:{item:FavoriteInput;onOpen:()=>void;unavailable?:boolean}) {
  ...
  return <article className="m-course-card"><button type="button" className="m-course-main" onClick={onOpen}><PlaceVisual imageUrl={first?.imageUrl} type={first?.type} detailType={first?.detailType} alt={item.title}/><div><span className="m-kicker"><MobileIcon name="pin"/>{copy(item.location||'동네 여행')}</span><h3>{copy(item.title)}</h3><p>{item.places.slice(0,3).map(place=>copy(place.name)).join(' → ')}</p><span className="m-course-duration"><MobileIcon name="clock"/>{uiText(durationLabel(item.places))}</span><span className="m-course-tags">{[...new Set(item.places.map(place=>place.category).filter(Boolean))].slice(0,2).map(tag=><span key={tag}>#{uiText(tag)}</span>)}</span></div></button><FavoriteButton item={item} compact/>{unavailable&&<p className="m-notice">{uiText("현재 이용 여부를 확인할 장소가 있어요.")}</p>}</article>;
}
