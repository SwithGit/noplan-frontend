...
export function MobileCourseCard({item,onOpen,unavailable=false}:{item:FavoriteInput;onOpen:()=>void;unavailable?:boolean}) {
  ...
  return <article className="m-course-card"><button type="button" className="m-course-main" onClick={onOpen}><PlaceVisual imageUrl={first?.imageUrl} type={first?.type} detailType={first?.detailType} alt={item.title}/><div><span className="m-kicker"><MobileIcon name="pin"/>{uiText(item.location||'동네 여행')}</span><h3>{uiText(item.title)}</h3><p>{item.places.slice(0,3).map(place=>place.name).join(' → ')}</p><span className="m-course-duration"><MobileIcon name="clock"/>{durationLabel(item.places)}</span><span className="m-course-tags">{[...new Set(item.places.map(place=>place.category).filter(Boolean))].slice(0,2).map(tag=><span key={tag}>#{tag}</span>)}</span></div></button><FavoriteButton item={item} compact/>{unavailable&&<p className="m-notice">{uiText("현재 이용 여부를 확인할 장소가 있어요.")}</p>}</article>;
}
...
export function MobileContentDetail({item,onClose,unavailable=false}:{item:FavoriteInput;onClose:()=>void;unavailable?:boolean}) {
  ...
  return <TripDialog title={uiText(item.title)} onClose={onClose}><div className="m-detail"><span className="m-kicker"><MobileIcon name="pin"/>{uiText(item.location||'장소 정보')}</span><p>{durationLabel(item.places)}{uiText(" · 이동시간 별도")}</p><FavoriteButton item={item}/><div className="m-detail-places">{item.places.map((place,index)=><article key={`${place.id}-${index}`}><PlaceVisual imageUrl={place.imageUrl} type={place.type} detailType={place.detailType} alt={place.name}/><div><small>{uiText(item.kind==='course'?`${index+1}번째 장소 · `:'')}{place.category}</small><h3>{place.name}</h3><p>{uiText(place.description)}</p><p>{place.address}</p>{item.kind==='course'&&<FavoriteButton item={placeFavorite(place)}/>}<a href={`https://map.kakao.com/link/search/${encodeURIComponent(`${place.name} ${place.address||''}`)}`} target="_blank" rel="noreferrer">{uiText("지도에서 확인 ")}<MobileIcon name="arrow"/></a></div></article>)}</div><p className="m-notice">{uiText(unavailable?'삭제되거나 추천에서 제외된 장소가 포함되어 있어요. 다른 코스를 골라주세요.':'저장된 장소 정보예요. 출발 전 영업시간과 실제 이동시간을 확인해 주세요.')}</p>{!desktop&&item.kind==='course'&&<button type="button" className="m-primary" disabled={unavailable} onClick={()=>{loadPlan({title:item.title,location:item.location,courseData:item.places,backupPlaces:[],source:'api',durationText:durationLabel(item.places)});onClose();navigate(ROUTES.courseMap);}}>{uiText("이 코스로 출발 ")}<MobileIcon name="arrow"/></button>}</div></TripDialog>;
}
