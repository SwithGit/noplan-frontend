...
export function MobileHome({user,active}:{user:UserSession|null;active:boolean}) {
  ...
  return <div className="mobile-page m-home">
    ...
    <p className="m-area-note">{uiText("주변 코스는 도보 기준이에요. 등록된 장소가 없으면 실시간 검색으로 찾으며 가격·리뷰 확인이 필요해요.")}</p>
    ...
    <Link className="m-promo" to={ROUTES.trips}><img src={travelMap} alt=""/><div><span>{uiText("noplan과 함께")}</span><strong>{uiText("계획 없어도")}<br/><em>{uiText("좋은 하루")}</em></strong><p>{uiText("새로운 발견은 생각보다 가까이")}</p></div><span className="m-promo-arrow"><MobileIcon name="arrow"/></span></Link>
