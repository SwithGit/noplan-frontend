...
export function MobileHome({user,active}:{user:UserSession|null;active:boolean}) {
  ...
  return <div className="mobile-page m-home">
    ...
    <Link className="m-promo m-promo-sea" to={ROUTES.trips}><img src={travelMap} alt=""/><div><span>{uiText("noplan과 함께")}</span><strong>{uiText("계획 없어도")}<br/><em>{uiText("좋은 하루")}</em></strong><p>{uiText("새로운 발견은 생각보다 가까이")}</p></div><span className="m-promo-arrow"><MobileIcon name="arrow"/></span></Link>
