...
export function DayRouteDialog({ document, dayId, disabled, onClose, onApply }: {
  ...
  return <>
    ...
    {pickerBlock && <PlacePicker needs={document.needs} destination={document.destination} initial={pickerAnchor} excluded={tripExclusions({ ...document, days: document.days.map(day => day.id === draft.id ? draft : day) }, undefined, pickerAnchor?.id)} context={uiText(`${shortDate(draft.date)} · ${pickerBlock.title}`)} onClose={() => setPickerId(null)} onSelect={place => {
