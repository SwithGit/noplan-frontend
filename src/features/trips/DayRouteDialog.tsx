...
export function DayRouteDialog({ document, dayId, disabled, onClose, onApply }: {
  ...
  return <>
    <TripDialog title="하루 동선 보기" className="day-route-dialog" onClose={close}>
      <div className="day-route-toolbar"><div><span className="trip-eyebrow">DAY {String(document.days.findIndex(day => day.id === dayId) + 1).padStart(2, '0')} · {document.destination}</span><strong>{shortDate(draft.date)}의 여정</strong><span>장소 사이의 이동을 살펴보고 하루 순서를 정해 보세요.</span></div><label>여행 중 이동<select aria-label="동선 이동수단" value={transport} onChange={event => setTransport(event.target.value as TripDocument['transport'])}>{Object.entries(transportLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label></div>
      <div className="day-route-layout">
        <section className="day-route-map-section" aria-label="하루 중심 장소 지도">
          <DayRouteMap points={points} activeId={activeId} onSelect={select} />
          <div className="day-route-map-summary"><span className="trip-eyebrow">오늘의 동선</span><strong>{points.length}개 중심 장소 <span>· {transportLabels[transport]}</span></strong><p>{loading ? '구간 사이 이동시간을 확인하고 있어요…' : allKnown ? `구간 사이 이동 약 ${durationText(successful.reduce((sum, route) => sum + route.durationMinutes!, 0))}${tightCount ? ` · 빠듯한 구간 ${tightCount}개` : ''}` : !legs.length ? '두 구간 이상 장소를 담으면 이동시간도 확인할 수 있어요.' : '이동시간이 확인되지 않은 구간을 살펴봐 주세요.'}</p></div>
          <div className="day-route-map-caption">점선은 방문 순서를 연결한 선이며 실제 도로 경로가 아니에요.</div>
        </section>
        <section className="day-route-list" aria-label="하루 방문 순서">
          <div className="day-route-list-heading"><h3>하루의 순서</h3><p>시간대는 유지하고, 주변 장소도 함께 옮겨요.</p></div>
          {currentResponse?.error && <div className="trip-alert" role="alert">{currentResponse.error}<button type="button" onClick={() => setAttempt(value => value + 1)}>이동시간 다시 조회</button></div>}
          {transport === 'transit' && <p className="trip-alert">대중교통 소요시간은 아직 연동되지 않았어요. 각 구간의 길찾기에서 확인해 주세요.</p>}
          {!blocks.length && <p className="trip-muted">일정 화면에서 구간을 먼저 추가해 주세요.</p>}
          {blocks.map((block, index) => {
            ...
            return <div key={block.id}>
              {leg && <div className={`day-route-transfer ${shortage ? 'tight' : ''}`} role="status"><TripIcon name="arrow" /><div><strong>{loading && leg.origin && leg.destination ? '이동시간 확인 중…' : known ? `${transportLabels[transport]} 약 ${durationText(route.durationMinutes!)}` : '이동시간 확인 필요'}{known && <span> · {(route.distanceMeters! / 1000).toFixed(1)}km</span>}</strong><p>{leg.from.name} → {leg.to.name}</p>{known ? <><small>{route.parkingMinutes ? `주차 여유 ${route.parkingMinutes}분 포함 · ` : ''}{shortage ? `다음 구간 시작까지 ${durationText(shortage)} 부족해요.` : `이동 후 ${durationText(leg.availableMinutes - route.durationMinutes!)} 여유가 있어요.`}</small><small>{route.source === 'same_location' ? '같은 위치' : route.source === 'kakao_driving' ? '카카오 자동차 경로' : route.source === 'tmap_pedestrian' ? 'TMAP 도보 경로' : 'Google 도보 경로'} · 현재 조회 기준</small></> : <small>{loading && leg.origin && leg.destination ? '경로를 조회하고 있어요.' : !leg.origin || !leg.destination ? '출발 또는 도착 장소에 좌표가 없어요.' : transport === 'transit' ? '대중교통 길찾기에서 확인해 주세요.' : '경로 정보를 확인하지 못했어요.'}</small>}{routeUrl && <a href={routeUrl} target="_blank" rel="noreferrer">카카오맵 길찾기 ↗</a>}{!loading && !known && transport !== 'transit' && leg.origin && leg.destination && !currentResponse?.error && <button className="trip-text-link" type="button" onClick={() => setAttempt(value => value + 1)}>다시 조회</button>}</div></div>}
              <article ref={element => { if (element) cards.current.set(block.id, element); else cards.current.delete(block.id); }} className={`day-route-card ${activeId === block.id ? 'active' : ''}`}>
                <div className="day-route-card-top"><span>{String(index + 1).padStart(2, '0')}</span><strong>{block.startTime} — {block.endTime}</strong><div><button className="trip-icon-button" type="button" aria-label={`${anchor?.name || block.title} 이전 시간대로`} disabled={disabled || index === 0} onClick={() => move(block.id, -1)}><TripIcon name="up" /></button><button className="trip-icon-button" type="button" aria-label={`${anchor?.name || block.title} 다음 시간대로`} disabled={disabled || index === blocks.length - 1} onClick={() => move(block.id, 1)}><TripIcon name="down" /></button></div></div>
                <button className="day-route-place-title" type="button" onClick={() => setActiveId(block.id)}><small>{block.title}</small><h3>{anchor?.name || '가고 싶은 장소를 담아보세요'}</h3></button>
                {anchor && <><p className="day-route-address">{anchor.address || '주소 정보 없음'}</p><div className="day-route-stops">{block.places.map(place => <span key={place.id}>{place.name}<small>{place.durationMinutes}분</small></span>)}</div></>}
                {overrun > 0 && <p className="trip-alert">체류·구간 내 이동이 시간대를 {durationText(overrun)} 넘어요. 일정 화면에서 시간을 조정해 주세요.</p>}
                <div className="day-route-card-footer"><button type="button" className="trip-text-link" disabled={disabled} onClick={() => setPickerId(block.id)}><TripIcon name="pin" />{anchor ? '장소 교체' : '중심 장소 담기'}</button>{anchor && <a className="trip-text-link" href={`https://map.kakao.com/link/search/${encodeURIComponent(`${anchor.name} ${anchor.address}`)}`} target="_blank" rel="noreferrer">장소 지도 ↗</a>}</div>
              </article>
            </div>;
          })}
    ...
    {pickerBlock && <PlacePicker destination={document.destination} initial={pickerAnchor} excluded={tripExclusions({ ...document, days: document.days.map(day => day.id === draft.id ? draft : day) }, undefined, pickerAnchor?.id)} context={`${shortDate(draft.date)} · ${pickerBlock.title}`} onClose={() => setPickerId(null)} onSelect={place => {
