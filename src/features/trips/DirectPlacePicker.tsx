...
export function DirectPlacePicker({ destination, initial, context, excluded, onClose, onRecommended, onSelect }: {
  ...
  return <TripDialog title="가고 싶은 곳을 직접 찾아보세요" className="direct-place-dialog" onClose={onClose}>
    <div className="direct-place-top"><div><span className="trip-eyebrow">{context}</span><p>네이버지도나 카카오맵에서 발견한 곳도 이름으로 찾아 담아보세요.</p></div><button type="button" className="trip-button" onClick={onRecommended}>추천 장소 보기</button></div>
    <div className="direct-place-workbench">
      <section className="direct-place-search" aria-label="직접 장소 검색">
        <div className="direct-place-tabs"><button type="button" aria-pressed={!manual} onClick={() => { cancelSearch(); setManual(false); setError(''); }}>이름으로 검색</button><button type="button" aria-pressed={manual} onClick={switchManual}>지도에 직접 지정</button></div>
        {!manual ? <><form className="direct-place-query" onSubmit={e => { e.preventDefault(); search(); }}><label htmlFor="direct-query">장소 이름·지역</label><div><input id="direct-query" autoFocus value={query} maxLength={80} placeholder="예: 울산 대왕암공원, 울산 카페" onChange={e => { cancelSearch(); setQuery(e.target.value); setPage(0); }} /><button type="submit" className="trip-button primary" disabled={!ready || busy}>검색</button></div><select aria-label="직접 검색 분류" value={category} onChange={e => { cancelSearch(); setCategory(e.target.value); setPage(0); }}><option value="">전체 장소</option><option value="FD6">음식점</option><option value="CE7">카페</option><option value="AT4">관광명소</option><option value="CT1">문화시설</option><option value="AD5">숙소</option></select></form>
          <div className="direct-place-results" aria-live="polite">{busy ? <p className="trip-muted">장소를 찾고 있어요…</p> : results.length ? results.map(place => { const reason = excludedPlace(place, excluded); return <button type="button" className={`direct-place-result ${selected?.id === place.id ? 'selected' : ''}`} key={place.id} disabled={Boolean(reason)} onClick={() => choose(place)}><span>{place.type}</span><strong>{place.name}</strong><small>{place.address}</small>{reason && <em>{reason}</em>}</button>; }) : <div className="direct-place-empty"><TripIcon name="pin" /><h3>{searched ? '검색된 장소가 없어요' : '목록에 없는 곳도 담아보세요'}</h3><p>{searched ? '지역이나 지점명을 함께 입력해 보세요. 찾는 곳이 없으면 지도에 직접 지정할 수 있어요.' : '음식점, 카페, 관광지, 숙소까지 장소 이름으로 검색할 수 있어요.'}</p><button type="button" className="trip-text-link" onClick={switchManual}>지도에 직접 지정하기 →</button></div>}</div>
          {page > 0 && <div className="direct-place-pages"><button type="button" className="trip-button" disabled={busy || page <= 1} onClick={() => search(page - 1)}>이전</button><span>{page} / {lastPage}</span><button type="button" className="trip-button" disabled={busy || page >= lastPage} onClick={() => search(page + 1)}>다음</button></div>}
          <small className="trip-muted">장소 검색 제공: 카카오맵</small></> : <div className="direct-place-manual"><h3>나만 아는 장소도 여행에</h3><p>주소로 지도를 이동하거나 지도를 눌러 정확한 위치를 지정해 주세요.</p><label>장소 이름<input maxLength={160} value={name} onChange={e => setName(e.target.value)} placeholder="여행에 표시할 이름" /></label><label>주소<input maxLength={300} value={address} onChange={e => { cancelSearch(); setAddress(e.target.value); setPin(undefined); }} placeholder="도로명 또는 지번 주소" /></label><button type="button" className="trip-button" disabled={!ready || busy} onClick={locateAddress}>주소로 위치 찾기</button><label>장소 분류<select value={type} onChange={e => { setType(e.target.value); if (e.target.value === '카페') setDuration(v => Math.max(30, v)); }}>{['관광지', '음식점', '카페', '문화시설', '쇼핑', '숙소', '기타'].map(value => <option key={value}>{value}</option>)}</select></label><div className="direct-pin-status">{pin ? '✓ 지도 위치를 지정했어요. 오른쪽 핀을 확인해 주세요.' : '오른쪽 지도에서 장소 위치를 눌러 주세요.'}</div></div>}
      </section>
      <section className="direct-place-preview" aria-label="선택 장소 위치"><div className="direct-place-map">{ready ? <Map center={center} level={6} style={{ width: '100%', height: '100%' }} onCreate={setMap} onClick={(_, event) => { if (manual) { cancelSearch(); setPin({ lat: event.latLng.getLat(), lng: event.latLng.getLng() }); setError(''); } }}><ZoomControl />{mapPin ? <MapMarker position={mapPin} /> : !manual && results.map(place => <MapMarker key={place.id} position={pointOf(place)!} title={place.name} onClick={() => { if (!excludedPlace(place, excluded)) choose(place); }} />)}</Map> : <div className="direct-place-empty"><TripIcon name="map" /><p>{failed ? '지도를 불러오지 못했어요. 잠시 후 창을 다시 열어 주세요.' : '카카오 지도를 연결하고 있어요…'}</p></div>}<span className="direct-map-hint">{manual ? '지도를 눌러 위치 지정' : '주소와 지점을 확인해 주세요'}</span></div>
        <form className="direct-place-confirm" onSubmit={apply}><div><span className="trip-eyebrow">{picked?.type || '선택한 장소'}</span><h3>{picked?.name || '마음에 드는 장소를 선택해 주세요'}</h3><p>{picked?.address || '지도에서 위치를 확인하고 일정에 담아요.'}</p>{picked && <div className="direct-place-links"><a href={picked.sourceUrl} target="_blank" rel="noreferrer">카카오맵 상세 ↗</a><a href={`https://map.naver.com/p/search/${encodeURIComponent(`${picked.name} ${picked.address}`)}`} target="_blank" rel="noreferrer">네이버지도에서 검색 ↗</a></div>}<small className="trip-muted">사진·운영시간·가격은 지도 상세에서 확인해 주세요.</small></div><label>머무는 시간<div><input aria-label="직접 선택 장소 체류시간" type="number" min={picked?.type === '카페' ? 30 : 10} max={600} step={1} required value={duration} onChange={e => setDuration(Number(e.target.value))} />분</div></label><button type="submit" className="trip-button primary" disabled={!picked || Boolean(duplicate) || busy}>{duplicate || (initial ? '이 장소로 변경하기' : '이 장소 일정에 담기')}<TripIcon name="arrow" /></button></form>
      </section>
    </div>{error && <p className="trip-alert direct-place-error" role="alert">{error}</p>}
  </TripDialog>;
}
