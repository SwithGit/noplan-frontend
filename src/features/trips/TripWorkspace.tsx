...
export function TripWorkspace({ user }: { user: UserSession | null }) {
  ...
  const [dialog, setDialog] = useState<'place' | 'block' | 'settings' | 'tourism' | 'sharing' | 'dayRoute' | 'planner' | 'overview' | null>(null);
  ...
  return <div className="trip-workspace">
    ...
    <div className="trip-editor-layout">
      ...
      <section className="trip-timeline" aria-label="날짜별 일정"><div className="trip-timeline-header"><div><span className="trip-eyebrow">DAY {String(document.days.indexOf(day) + 1).padStart(2, '0')}</span><h2>{shortDate(day.date)}의 여행</h2><small className="trip-muted">{transportLabels[day.transport || document.transport]}</small></div><div className="trip-timeline-actions">{ulsanPlanner && <><button className="trip-button primary" type="button" disabled={saving} onClick={() => openPlanner()}><TripIcon name="spark" />노피의 코스플래닝</button></>}<button className="trip-button" type="button" disabled={saving} onClick={() => setDialog('dayRoute')}><TripIcon name="map" />하루 동선 보기</button><button className="trip-button" onClick={addBlock} disabled={saving || day.blocks.length >= 12} type="button"><TripIcon name="plus" />구간 추가</button></div></div>
        <div className="trip-timeline-list">{[...day.blocks].sort((a, b) => a.startTime.localeCompare(b.startTime)).map((segment, index) => <article className={`trip-segment ${segment.id === block?.id ? 'selected' : ''}`} key={segment.id}>
          ...
          <div className="trip-segment-card"><div className="trip-segment-heading"><div className="trip-segment-heading-copy"><button className="trip-segment-title" type="button" onClick={() => chooseBlock(segment)} aria-pressed={segment.id === block?.id}><div><span>{segment.places.length ? `${segment.places.length}곳 · 체류 ${segment.places.reduce((sum, place) => sum + place.durationMinutes, 0)}분` : '이 시간을 무엇으로 채울까요?'}</span><h3>{segment.places.find(place => place.tourism)?.name || segment.title}</h3></div><TripIcon name="arrow" /></button>
      ...
      <aside ref={inspector} className={`trip-inspector ${mobileDetail ? 'mobile-open' : ''}`} aria-label="선택 구간 상세"><div className="trip-inspector-top"><img src={nopi} alt="" /><div><span>작은 계획, 좋은 여행</span><strong>{SHOW_BLOCK_RECOMMENDATIONS ? '노피와 함께 채워요' : '여행 장소를 살펴봐요'}</strong></div><button className="trip-icon-button trip-mobile-close" aria-label="구간 상세 접기" type="button" onClick={() => setMobileDetail(false)}><TripIcon name="close" /></button></div>
        ...
        {block ? <div className="trip-inspector-body"><div className="trip-selected-summary"><span>{block.startTime} — {block.endTime}</span><h3>{tourismAnchor?.name || block.title}</h3><p>{shortDate(day.date)} · {block.places.length}개 장소</p></div>
          <section className="tourism-anchor-panel"><span className="trip-eyebrow">중심 관광지</span><strong>{tourismAnchor?.name || '이 구간에서 꼭 가고 싶은 곳'}</strong><p className="trip-muted">{tourismAnchor ? `머무는 시간 ${tourismAnchor.durationMinutes}분 · 방문 전 운영시간과 요금을 확인해 주세요.` : '가고 싶은 관광지나 음식점을 골라 일정에 담아보세요.'}</p><button type="button" className="trip-button" disabled={saving} onClick={() => setDialog('tourism')}>{tourismAnchor ? '관광지·관람시간 변경' : '중심 관광지 담기'}</button>{tourismAnchor && <button className="trip-text-link" type="button" disabled={saving} onClick={() => updateBlock({ ...block, area: block.places.find(place => !place.tourism)?.address.slice(0, 160) || document.destination, places: block.places.filter(place => !place.tourism) })}>중심 관광지 제거</button>}</section>
    ...
    {dialog === 'planner' && <NopiCoursePlanner key={day.id} document={document} dayId={day.id} disabled={saving || conflict || blocked} onClose={() => setDialog(null)} onApply={(nextDay, transport, baseline, next) => {
      ...
      change({ ...document, days: document.days.map(item => item.id === nextDay.id ? { ...nextDay, transport } : item) });
      setBlockId(nextDay.blocks[0]?.id || '');
      const followingDay = document.days[document.days.findIndex(item => item.id === nextDay.id) + 1];
      if (next && followingDay) { setDayId(followingDay.id); }
      else setDialog(next ? 'overview' : null);
    }} />}
