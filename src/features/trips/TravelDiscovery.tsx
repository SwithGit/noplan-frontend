...
export function TravelDiscovery({ destination, anchorId, label, onSelect }: { destination: string; anchorId?: string; label?: string; onSelect: (place: TourismAttraction) => void }) {
  ...
  return <details className="support-discovery" open={expanded} onToggle={e => setExpanded(e.currentTarget.open)}><summary>{t(label || (anchorId ? '함께 둘러볼 관광지' : '이 지역의 중심 관광지에서 시작하기'))}</summary>
