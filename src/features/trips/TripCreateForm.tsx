...
export function TripCreateForm({ user }: { user: UserSession | null }) {
  ...
  const create = (event: FormEvent) => {
    ...
    try {
      ...
      if (attraction) {
        if (attraction.contentTypeId === '25') throw new Error('여행코스 전체 대신 꼭 방문할 개별 장소를 선택해 주세요.');
        if (!targetDay || !targetBlock) throw new Error('관광지를 방문할 날짜를 여행 기간 안에서 다시 선택해 주세요.');
        trip.document = setTourismAnchor(trip.document, targetDay.id, targetBlock.id, attraction, visitDuration, true);
      }
  ...
  return <>
    <form id="trip-create" className="trip-create-form" onSubmit={create}>
      ...
      <div className="trip-home-tourism"><div><span className="trip-eyebrow">{uiText("꼭 방문할 장소 · 선택사항")}</span><h3>{uiText(attraction?.name || '꼭 가고 싶은 장소가 있나요?')}</h3><p>{uiText(attraction ? `${attraction.address} · 관람 ${visitDuration}분` : '장소를 고르면 다음 화면에서 노피가 방문 시간대를 유지하며 주변 코스를 채워드려요.')}</p></div><div className="tourism-home-actions"><button className="trip-button" type="button" disabled={!selectedDestination} onClick={() => setPickingTourism(true)}><TripIcon name="pin" />{uiText(attraction ? '관광지 변경' : '검색해서 고르기')}</button>{attraction && <button className="trip-text-link" type="button" onClick={() => setAttraction(undefined)}>{uiText("선택 해제")}</button>}</div>{attraction && <div className="trip-form-row tourism-home-schedule"><label className="trip-field">{uiText("방문 날짜")}<input type="date" required min={startDate} max={endDate} value={visitDate || startDate} onChange={e => setVisitDate(e.target.value)} /></label><label className="trip-field">{uiText("방문 구간")}<select value={visitSlot} onChange={e => setVisitSlot(Number(e.target.value))}><option value={0}>{uiText("오전 · 09:00–12:00")}</option><option value={1}>{uiText("오후 · 13:00–17:00")}</option><option value={2}>{uiText("저녁 · 18:00–21:00")}</option></select></label></div>}<TravelDiscovery destination={destination} label="추천 관광지에서 고르기" onSelect={place => { setAttraction(place); setPickingTourism(true); }} /></div>
    ...
    {pickingTourism && <TourismPicker needs={needs} destination={destination} initial={attraction} initialDuration={visitDuration} context={uiText("선택할 여행 구간")} onClose={() => setPickingTourism(false)} onSelect={(place, duration) => { if (place.contentTypeId === '25') throw new Error('여행코스 전체 대신 꼭 방문할 개별 장소를 선택해 주세요.'); setAttraction(place); setVisitDuration(duration); setPickingTourism(false); }} />}
