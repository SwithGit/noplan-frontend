  ...
  return <>
    <form id="trip-create" className="trip-create-form" onSubmit={create}>
      ...
      <div className="trip-create-fields">
        <div className="trip-destination" role="group" aria-label="어디로 떠날까요?">
          <span className="trip-destination-heading"><TripIcon name="pin" /> 어디로 떠날까요?</span>
          {regions.length > 0 ? <div className="trip-destination-selects">
            <TripDetailSelect label="시·도" icon="pin" value={selectedDestination?.region.id || ''}
            ...
            {selectedDestination ? <TripDetailSelect key={selectedDestination.region.id} label="시·군·구" icon="pin" value={selectedDestination.district}
              ...
              : <span className="trip-destination-pending">시·도를 먼저 선택</span>}
          </div> : <div className="trip-destination-status" role="status">{regionError ? <>지역을 불러오지 못했어요. <button type="button" onClick={() => { setRegionError(false); setRegionAttempt(value => value + 1); }}>다시 시도</button></> : '지역을 불러오는 중…'}</div>}
        </div>
      ...
      {regions.length > 0 && !selectedDestination && <p className="trip-destination-notice" role="status">기존 여행지 ‘{destination}’의 시·도와 시·군·구를 다시 선택해 주세요.</p>}
