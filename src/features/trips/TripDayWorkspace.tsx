...
export function TripDayWorkspace({ document, day, photos, disabled, onChange, onDay, onPlanner, onSettings, onOverview, onRoute, onAdd, onEdit, onBlock, onAddBlock }: {
  ...
  return <div className="journey-day-layout">
    <section className="journey-day-editor" aria-label={uiText("하루 일정 편집")}>
      ...
      <div className="journey-main-actions"><button className="trip-button primary" type="button" disabled={disabled} onClick={onPlanner}><img src={nopi} alt="" />{uiText("노피의 코스플래닝")}</button><button className="trip-button" type="button" disabled={disabled} onClick={() => onAdd()}><TripIcon name="plus" />{uiText("직접 장소 추가")}</button><button className="trip-button journey-overview-button" type="button" onClick={onOverview}><TripIcon name="map" />{uiText("전체 일정 보기")}</button></div>
      ...
      {showReason && <div className="journey-reason"><p>{uiText("노피는 선택한 여행 목적과 성·연령 추천 기준, 장소 사이 이동거리를 함께 고려해 코스를 만들어요. 추천 조건은 노피의 코스플래닝에서 변경할 수 있어요.")}</p><p>{uiText(transport === 'transit' ? '대중교통 자동 추천은 준비 중이에요.' : `자동 추천은 실제 경로 ${courseDistanceLabel(transport)} 이내로 연결해요.`)}{uiText(" 직접 추가한 장소는 거리 제한을 넘으면 안내해요.")}</p><p>{uiText("지도의 젤리 선은 방문 순서예요. 이동시간은 현재 조회 기준이며, 장소를 옮겨도 예약한 시간대는 유지돼요.")}</p></div>}
            ...
            <article ref={element => { if (element) cards.current.set(place.id, element); else cards.current.delete(place.id); }} className={`journey-stop ${activeId === place.id ? 'selected' : ''}`}>
              ...
              <div className="journey-stop-card">
                ...
                <div className="journey-stop-actions"><button type="button" className="trip-text-link" disabled={disabled} onClick={() => onEdit(block, place)}>{uiText("상세·장소 변경")}</button><button type="button" className="trip-text-link" disabled={disabled} onClick={() => onBlock(block)}>{uiText("시간·메모 수정")}</button><div>
                  {!place.tourism && !place.event && <button type="button" className="trip-icon-button" disabled={disabled} aria-label={uiText(`$<TourismText place={place} /> ${place.fixed ? '고정 해제' : '고정'}`)} onClick={() => changeDay({ ...day, blocks: day.blocks.map(item => item.id === block.id ? { ...item, places: item.places.map(p => p.id === place.id ? { ...p, fixed: !p.fixed, requiredVisit: p.fixed ? undefined : p.requiredVisit } : p) } : item) })}><TripIcon name={place.fixed ? 'lock' : 'unlock'} /></button>}
