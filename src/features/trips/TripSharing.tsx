...
export function TripSharing({ trip, onClose }: { trip: TripRecord; onClose: () => void }) {
  ...
  return <TripDialog title="친구와 함께 일정 짜기" className="trip-sharing-dialog" onClose={onClose}>
    <p className="trip-muted">참여한 친구는 장소·시간·메모를 함께 수정할 수 있어요. 변경사항은 자동 저장되고 약 3초 안에 서로 반영돼요.</p>
    <h3>함께하는 사람 {members.length}명</h3>
    <ul className="trip-member-list">{members.map(member => <li key={member.userId}><span><strong>{member.nickname}</strong><small>{member.role === 'owner' ? '여행 만든 사람' : '함께 편집'}</small></span>{owner && member.role !== 'owner' && <button disabled={busy} className="trip-text-link" onClick={() => {
    ...
    {owner ? <div className="trip-invite-controls">
      <p className="trip-muted">초대 링크를 받은 사람은 로그인 후 참여할 수 있어요. 링크는 7일 동안 유효하고, 새로 만들면 이전 링크는 만료돼요.</p>
      ...
      })}>{busy ? '처리 중…' : link ? '새 초대 링크 만들기' : '초대 링크 만들기'}</button>
      {link && <><label className="trip-field">친구 초대 링크<input aria-label="친구 초대 링크" value={link} readOnly onFocus={event => event.currentTarget.select()} /></label><button className="trip-button" onClick={() => void perform(async () => { await navigator.clipboard.writeText(link); setMessage('초대 링크를 복사했어요.'); })}>링크 복사</button><small>{new Date(expires).toLocaleDateString('ko-KR')}까지 참여 가능</small></>}
      <button className="trip-text-link" disabled={busy} onClick={() => void perform(async () => { await revokeTripInvite(trip.id); setLink(''); setMessage('기존 초대 링크를 만료했어요. 이미 참여한 친구의 편집 권한은 유지돼요.'); })}>초대 링크 만료시키기</button>
    </div> : <p className="trip-muted">새 친구 초대와 참여자 관리는 여행을 만든 사람이 할 수 있어요.</p>}
    {message && <p className="trip-alert" role="status">{message}</p>}
  </TripDialog>;
}
...
export function TripJoin({ user }: { user: UserSession | null }) {
  ...
  return <div className="trip-load-state trip-join"><span className="trip-eyebrow">LET’S PLAN TOGETHER</span><h1>친구와 같은 여행 노트에 초대됐어요</h1>
    <p>참여하면 서로 추가한 장소와 바꾼 일정이 함께 반영돼요.</p>
    {!valid ? <p role="alert">올바르지 않은 초대 링크예요. 친구에게 링크를 다시 받아 주세요.</p>
      : !user ? <Link className="trip-button primary" to={ROUTES.login} onClick={rememberInvite}>로그인하고 참여하기</Link>
        ...
        }}>{busy ? '참여 중…' : `${user.userNick}님으로 함께 편집하기`}</button>}
    {error && <p role="alert">{error} <Link to={ROUTES.login} onClick={rememberInvite}>다시 로그인</Link></p>}
    <Link to={ROUTES.trips} className="trip-text-link" onClick={() => { try { sessionStorage.removeItem(PENDING_TRIP_INVITE); } catch { /* Optional. */ } }}>내 여행으로</Link>
  </div>;
}
