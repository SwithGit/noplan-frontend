...
export function setTourismAnchor(document: TripDocument, dayId: string, blockId: string, attraction: TourismAttraction, durationMinutes: number): TripDocument {
  ...
  if (!/^\d{1,20}$/.test(attraction.contentId) || !['12', '14', '28'].includes(attraction.contentTypeId) || !attraction.name || !Number.isFinite(attraction.lat) || !Number.isFinite(attraction.lng) || attraction.lat < 33 || attraction.lat > 39 || attraction.lng < 124 || attraction.lng > 132) throw new Error('관광지 위치를 확인할 수 없어요. 다시 검색해 주세요.');
