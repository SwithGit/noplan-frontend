...
export function TripHome({ user, libraryOnly=false }: { user: UserSession | null; libraryOnly?:boolean }) {
  ...
  const deleteButton = (trip: TripRecord, guest = false) => <button className="trip-library-delete" type="button" aria-label={uiText(`${trip.document.title} ${trip.collaboration?.role === 'editor' ? '나가기' : '삭제'}`)} onClick={() => void beginDelete(trip, guest)}>{uiText(trip.collaboration?.role === 'editor' ? '나가기' : '삭제')}</button>;
  useEffect(() => {
    if (!user) return;
    let cancelled = false;
  ...
  return <div className={`trip-home ${libraryOnly?'library-only':''}`}>
    ...
    {error && <div className="trip-alert" role="alert">{uiText(error)}{user && <button onClick={() => { setLoading(true); setReload(value => value + 1); }} type="button">{uiText("다시 불러오기")}</button>}</div>}
    {notice && <p className="trip-library-notice" role="status">{uiText(notice)}</p>}
    <section className="trip-library"><header><div><span className="trip-eyebrow">MY JOURNEYS</span><h2>{uiText("다음 여행이 기다리고 있어요")}</h2></div>{libraryOnly?<><Link className="trip-button primary m-desktop-only" to={ROUTES.newTrip}>{uiText("새 여행 만들기")}</Link><span className="trip-muted m-mobile-only">{uiText(user ? `${user.userNick}님의 여행` : '나만의 여행 노트')}</span></>:<span className="trip-muted">{uiText(user ? `${user.userNick}님의 여행` : '나만의 여행 노트')}</span>}</header>
      <div className="trip-library-grid">
        {drafts.map(draft => <article className="trip-library-item" key={draft.id}><button className="trip-library-card draft" type="button" onClick={() => navigate(tripRoute(draft.id), { state: { initialTrip: draft } })}><span className="trip-card-art"><TripIcon name="map" /><span>{draft.version ? 'JOURNEY' : 'DRAFT'}</span></span><div><span className="trip-tag">{uiText(draft.collaboration?.enabled ? `친구와 함께 · ${draft.collaboration.memberCount}명` : draft.version ? '계정에 저장한 여행 · 이 기기의 작업본' : '이 브라우저의 초안')}</span><h3>{uiText(draft.document.title)}</h3><p>{shortDate(draft.document.startDate)} · {tripLength(draft.document)}</p><span className="trip-card-link">{uiText("이어서 계획하기 ")}<TripIcon name="arrow" /></span></div></button>{deleteButton(draft)}</article>)}
        {guestDrafts.filter(trip => !drafts.some(draft => draft.id === trip.id) && !trips.some(saved => saved.id === trip.id)).map(trip => <article className="trip-library-item" key={trip.id}><button className="trip-library-card" type="button" onClick={() => navigate(tripRoute(trip.id), { state: { initialTrip: trip } })}><span className="trip-card-art"><TripIcon name="map" /></span><div><span className="trip-tag">{uiText("로그인 전에 만든 초안")}</span><h3>{uiText(trip.document.title)}</h3><span className="trip-card-link">{uiText("이 계정으로 이어서 작성 ")}<TripIcon name="arrow" /></span></div></button>{deleteButton(trip, true)}</article>)}
        {trips.filter(trip => !drafts.some(draft => draft.id === trip.id)).map(trip => <article className="trip-library-item" key={trip.id}><Link to={tripRoute(trip.id)} className="trip-library-card"><span className="trip-card-art"><TripIcon name="pin" /><span>{trip.document.destination}</span></span><div><span className="trip-tag">{uiText(trip.collaboration?.enabled ? `친구와 함께 · ${trip.collaboration.memberCount}명` : '저장한 여행')}</span><h3>{uiText(trip.document.title)}</h3><p>{shortDate(trip.document.startDate)} · {tripLength(trip.document)}</p><span className="trip-card-link">{uiText("일정 열기 ")}<TripIcon name="arrow" /></span></div></Link>{deleteButton(trip)}</article>)}
        {!drafts.length && !trips.length && !guestDrafts.length && <div className="trip-library-empty"><span className="trip-empty-icon"><TripIcon name="map" /></span><h3>{uiText(loading ? '여행 노트를 불러오고 있어요' : '아직 빈 여행 노트, 곧 특별해질 거예요.')}</h3><p>{uiText(user ? (libraryOnly ? '새 여행 만들기에서 목적지와 날짜를 골라보세요.' : '위에서 목적지와 날짜를 고르면 첫 페이지가 시작돼요.') : '로그인 없이 초안을 만들 수 있어요. 계정에 저장하면 다른 기기에서도 이어볼 수 있어요.')}</p>{!user && <Link to={ROUTES.login} className="trip-text-link">{uiText("로그인하기 ")}<TripIcon name="arrow" /></Link>}</div>}
      </div>
    </section>
    {removing && <TripDialog title={uiText(removing.saved && removing.trip.collaboration?.role === 'editor' ? '공동 여행에서 나갈까요?' : '이 여행을 삭제할까요?')} onClose={() => { if (!deleteBusy) setRemoving(undefined); }} className="trip-delete-dialog">
      <div className="trip-delete-content"><strong>{uiText(removing.trip.document.title)}</strong><p>{shortDate(removing.trip.document.startDate)} · {tripLength(removing.trip.document)}</p>
        <p>{uiText(!deleteReady && deleteBusy ? '저장된 여행 정보를 확인하고 있어요.' : removing.saved ? removing.trip.collaboration?.role === 'editor' ? '내 목록에서 빠지고 공동 편집이 종료돼요. 다른 참여자의 여행은 유지돼요.' : `계정에 저장된 일정과 이 기기의 작업본을 삭제해요.${removing.trip.collaboration?.enabled ? ' 함께한 친구들도 이 여행에 접근할 수 없어요.' : ''} 삭제 후에는 되돌릴 수 없어요.` : '이 브라우저에 보관한 초안을 삭제해요. 삭제 후에는 되돌릴 수 없어요.')}</p>
        {deleteError && <p role="alert" className="trip-delete-error">{deleteError}</p>}
        <div className="trip-delete-actions"><button className="trip-button" type="button" disabled={deleteBusy} onClick={() => setRemoving(undefined)}>{uiText("취소")}</button><button className="trip-button trip-delete-confirm" type="button" disabled={deleteBusy || !deleteReady} onClick={() => void confirmDelete()}>{uiText(deleteBusy ? '처리 중…' : removing.saved && removing.trip.collaboration?.role === 'editor' ? '여행에서 나가기' : '삭제하기')}</button></div>
      </div>
    </TripDialog>}
    <div className="trip-how"><div><span>01</span><h3>{uiText("큰 일정부터 가볍게")}</h3><p>{uiText("날짜와 오전·오후의 흐름을 정해요.")}</p></div><div><span>02</span><h3>{uiText("꼭 갈 곳은 고정해두기")}</h3><p>{uiText("놓치고 싶지 않은 장소를 담아두세요.")}</p></div><div><span>03</span><h3>{uiText("빈 시간은 노피와 함께")}</h3><p>{uiText("선택한 구간에 주변 코스를 더해요.")}</p></div></div>
  </div>;
}
