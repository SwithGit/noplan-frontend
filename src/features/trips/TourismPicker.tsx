...
const categories = [{ id: '12', name: '관광지' }, { id: '14', name: '문화시설' }, { id: '28', name: '레포츠' }, { id: '25', name: '여행코스' }, { id: '38', name: '쇼핑' }] as const;
...
export function TourismPicker({ destination, initial, initialDuration = 90, context, onClose, onSelect }: {
  ...
  const regionKeyword = ulsan ? '울산' : destination.trim();
  ...
  const [query, setQuery] = useState<SearchQuery>({ keyword: regionKeyword, type: ulsan ? 'all' : '12', page: 1, ...(ulsan ? { region: 'ulsan', sort: 'recommended', profile: 'member' } : {}) });
  ...
  const search = (event: FormEvent) => {
    ...
    setQuery({ ...query, keyword: keyword.trim() || regionKeyword, page: 1 });
  };
  const showRegion = () => {
    setKeyword('');
    setQuery({ ...query, keyword: regionKeyword, type: ulsan ? 'all' : '12', page: 1 });
  };
  ...
  return <TripDialog title="어디를 여행의 중심으로 할까요?" className="tourism-dialog" onClose={onClose}>
    <div className="tourism-intro"><span className="tourism-context"><TripIcon name="calendar" />{context}</span><p>마음에 드는 곳을 고르면, 그 주변으로 여행을 이어갈 수 있어요.</p></div>
    <div className="tourism-workbench">
      <section className="tourism-discovery" aria-label="관광지 찾아보기">
        ...
        <div className="tourism-discovery-toolbar">
          <div className="tourism-categories" aria-label="관광지 종류">{categoryOptions.map(category => <button type="button" key={category.id} aria-pressed={query.type === category.id} onClick={() => setQuery({ ...query, keyword: keyword.trim() || regionKeyword, type: category.id as SearchQuery['type'], page: 1 })}>{category.name}</button>)}</div>
          {ulsan ? <div className="tourism-sort-tabs" aria-label="관광지 정렬">{([{ id: 'recommended', label: '맞춤 추천순' }, { id: 'popular', label: '인기순' }, { id: 'name', label: '이름순' }] as const).map(sort => <button key={sort.id} type="button" aria-pressed={query.sort === sort.id} onClick={() => setQuery({ ...query, sort: sort.id, page: 1 })}>{sort.label}</button>)}</div> : <span className="tourism-source">한국관광공사 제공</span>}
        </div>
      ...
      <aside className="tourism-detail" aria-label="선택한 관광지">
        {current ? <form onSubmit={apply} className="tourism-detail-form"><div className="tourism-detail-scroll"><span className="trip-eyebrow">이번 구간의 중심</span><AttractionPhoto place={current} large /><span className="tourism-detail-type">{current.type}</span><h3>{current.name}</h3><p className="tourism-detail-address">{current.address}</p><a className="trip-text-link" href={`https://map.kakao.com/link/map/${encodeURIComponent(current.name)},${current.lat},${current.lng}`} target="_blank" rel="noreferrer">{current.contentTypeId === '25' ? '코스 대표 위치 확인' : '지도에서 위치 확인'} <TripIcon name="arrow" /></a>{current.contentTypeId === '25' && <p className="tourism-course-note"><strong>코스의 대표 위치를 중심으로 담아요.</strong> 여러 장소를 묶은 여행코스예요. 코스 전체 경유지가 자동으로 추가되지는 않으며, 주변 추천도 이 대표 위치를 기준으로 찾아요. 이동을 포함해 이번 구간에서 쓸 시간을 정해 주세요.</p>}<div className="tourism-next-step"><TripIcon name="spark" /><p>방문 후 남는 시간에는<br /><strong>주변 맛집·카페 코스</strong>를 더해보세요.</p></div><p className="tourism-detail-note">입장료·운영시간·휴무는 방문 전에 확인해 주세요.</p></div><div className="tourism-apply"><div className="tourism-duration"><label htmlFor="tourism-duration">얼마나 머무를까요?<span><input id="tourism-duration" type="number" required min={10} max={600} step={5} value={duration} onChange={e => { setDuration(Number(e.target.value)); setApplyError(''); }} />분</span></label><div className="tourism-duration-options">{[60, 90, 120].map(value => <button type="button" aria-pressed={duration === value} key={value} onClick={() => { setDuration(value); setApplyError(''); }}>{value === 60 ? '1시간' : value === 90 ? '1시간 30분' : '2시간'}</button>)}</div></div>{applyError && <p className="trip-alert" role="alert">{applyError}</p>}<button className="trip-button primary" type="submit">{initial ? '이곳으로 변경하기' : '이곳 일정에 담기'}<TripIcon name="arrow" /></button><small>선택한 구간의 첫 일정으로 고정돼요.</small></div></form> : <div className="tourism-detail-empty"><span className="tourism-empty-orbit"><TripIcon name="pin" /></span><span className="trip-eyebrow">A PLACE TO START</span><h3>여행의 중심이 될<br />한 곳을 골라보세요.</h3><p>왼쪽에서 관광지를 선택하면<br />관람시간을 정하고 일정에 담을 수 있어요.</p><div><span>01</span> 마음에 드는 관광지 선택</div><div><span>02</span> 머무는 시간 정하기</div><div><span>03</span> 내 여행에 담기</div></div>}
      </aside>
    </div>
    <footer className="tourism-dialog-footer"><span>{ulsan ? '울산' : '전국'} 관광지 · 문화시설 · 레포츠 · 여행코스 · 쇼핑</span><a href="https://www.data.go.kr/data/15101578/openapi.do" target="_blank" rel="noreferrer">관광정보·사진 출처: 한국관광공사 TourAPI ↗</a></footer>
  </TripDialog>;
}
