...
export function TourismPicker({ destination, initial, initialDuration = 90, context, disabledPlaces = {}, onClose, onSelect }: {
  ...
  const apply = (event: FormEvent) => {
    event.preventDefault(); if (!current || disabledPlaces[current.contentId]) return;
  ...
  return <TripDialog title="어디를 여행의 중심으로 할까요?" className={`tourism-dialog ${detailExpanded ? 'tourism-expanded' : ''}`} onClose={onClose}>
    ...
    <div className="tourism-workbench">
      ...
      <aside className="tourism-detail" aria-label="선택한 관광지">
        {current ? <form onSubmit={apply} className="tourism-detail-form">
          ...
          <div className="tourism-apply"><div className="tourism-duration"><label htmlFor="tourism-duration">{current.contentTypeId === '25' ? '이번 구간에 얼마나 배정할까요?' : '얼마나 머무를까요?'}<span><input id="tourism-duration" type="number" required min={10} max={600} step={5} value={duration} onChange={e => { setDuration(Number(e.target.value)); setApplyError(''); }} />분</span></label><div className="tourism-duration-options">{[60, 90, 120].map(value => <button type="button" aria-pressed={duration === value} key={value} onClick={() => { setDuration(value); setApplyError(''); }}>{value === 60 ? '1시간' : value === 90 ? '1시간 30분' : '2시간'}</button>)}</div></div>{applyError && <p className="trip-alert" role="alert">{applyError}</p>}<button className="trip-button primary" type="submit" disabled={Boolean(disabledPlaces[current.contentId])}>{initial ? '이곳으로 변경하기' : '이곳 일정에 담기'}<TripIcon name="arrow" /></button><small>선택한 구간의 첫 일정으로 고정돼요.</small></div>
    ...
    <footer className="tourism-dialog-footer"><span>{ulsan ? '울산' : '전국'} 관광지 · 문화시설 · 레포츠 · 여행코스 · 쇼핑 · 음식점</span><a href="https://www.data.go.kr/data/15101578/openapi.do" target="_blank" rel="noreferrer">관광정보·사진 출처: 한국관광공사 TourAPI ↗</a></footer>
  </TripDialog>;
}
