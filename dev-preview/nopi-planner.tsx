...
createRoot(document.getElementById('root')!).render(<div style={{ background: '#fcfaff', minHeight: '100vh' }}>
  ...
  <MemoryRouter initialEntries={new URLSearchParams(window.location.search).has('home') ? ['/app/trips/new'] : [{ pathname: `/app/trips/${sample.id}`, state: { initialTrip: sample } }]}><Routes><Route path="/app/trips/new" element={<TripHome user={null} />} /><Route path="/app/trips" element={<TripHome user={null} libraryOnly />} /><Route path="/app/trips/:id" element={<TripWorkspace user={null} />} /></Routes></MemoryRouter>
</div>);
